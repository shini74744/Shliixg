...
#   2. 只显示本次哪吒相关入侵特征
#   3. 乱码进程直接判定为疑似感染
#   4. 避免普通 cron、普通 /tmp、普通 curl/wget、authorized_keys 误报
#
# 用法：
#   bash nezhajc1.sh
#   bash nezhajc1.sh --show-ok
...
MAL_FILE_RE='SystemLog|/tmp/b$|/tmp/b[[:space:]]|download\.sh|ice\.sh|harvest\.sh|xmrig\.sh|xmrig|kinsing|kdevtmpfsi|recvp\.php'

MAL_CMD_RE='curl.*\|.*sh|curl.*\|.*bash|wget.*\|.*sh|wget.*\|.*bash|curl.*download\.sh|curl.*ice\.sh|curl.*harvest\.sh|curl.*xmrig\.sh|wget.*download\.sh|wget.*ice\.sh|wget.*harvest\.sh|wget.*xmrig\.sh|/tmp/SystemLog|/tmp/b([[:space:]]|$)|download\.sh|ice\.sh|harvest\.sh|xmrig\.sh|xmrig|stratum|recvp\.php|mkdir -p /root/\.ssh|chmod 700 /root/\.ssh'
  ...
  C_NEZHA_INFO="$(run_in_container "$cname" '
    {
      ...
      find /etc/systemd/system /lib/systemd/system /etc/init.d /etc/runlevels -maxdepth 4 \
        -iname "*nezha*" -ls 2>/dev/null | head -80

      echo
      echo "[agent-config]"
      find /opt/nezha /etc/nezha -maxdepth 5 -type f 2>/dev/null | head -100
    }
  ' || true)"
...
[哪吒小鸡] ${cname}"
    ...
    [ "$SHOW_OK" -eq 1 ] && ok "$cname 未发现哪吒 agent，仅检查乱码进程和强感染特征"
  fi
  ...
  if [ -n "$C_PROC_BAD" ]; then
    ...
    [ "$SHOW_OK" -eq 1 ] && ok "$cname 当前进程未命中哪吒被黑强特征"
  fi
  ...
  if [ -n "$C_TMP_BAD" ]; then
    ...
    [ "$SHOW_OK" -eq 1 ] && ok "$cname 临时目录未发现哪吒被黑相关文件"
  fi
  ...
  if [ -n "$C_NEZHA_BAD" ]; then
    ...
    [ "$SHOW_OK" -eq 1 ] && ok "$cname 哪吒目录未命中被黑相关特征"
  fi

  echo
  echo "----- cron：只检查强感染命令，不检查普通 curl/wget/tmp -----"
  ...
  if [ -n "$C_CRON_BAD" ]; then
    ...
    [ "$SHOW_OK" -eq 1 ] && ok "$cname cron 未命中哪吒被黑强特征"
  fi

  echo
  echo "----- 启动项：只检查强感染命令，不检查普通 curl/wget/tmp -----"
  ...
  if [ -n "$C_START_BAD" ]; then
    ...
    [ "$SHOW_OK" -eq 1 ] && ok "$cname 启动项未命中哪吒被黑强特征"
  fi
  ...
  if [ -n "$C_PRELOAD_BAD" ]; then
    echo "$C_PRELOAD_BAD"
    echo "$C_PRELOAD_BAD" | grep -Eia 'usranalyse|libusranalyse|xmrig|miner|SystemLog|/tmp|/dev/shm|/var/tmp' >/dev/null
    if [ $? -eq 0 ]; then
      add_reason "ld.so.preload 命中恶意特征"
    else
      C_WARN_REASON="${C_WARN_REASON}；存在 ld.so.preload 需人工确认"
    fi
  else
    [ "$SHOW_OK" -eq 1 ] && ok "$cname /etc/ld.so.preload 不存在"
  fi
  ...
  if echo "$C_CONN" | grep -q "NO_NET_TOOL=1"; then
    C_WARN_REASON="${C_WARN_REASON}；缺少 ss/netstat 无法统计连接"
  else
    ...
    if [ "$UNIQUE_PUBLIC_IP" -ge "$CONN_THRESHOLD" ] 2>/dev/null; then
      ...
      [ "$SHOW_OK" -eq 1 ] && ok "$cname 公网连接数量未超过阈值"
    fi
  fi
...
echo
echo "判定规则："
echo "1. 本脚本只针对这次哪吒 agent 被黑相关特征做专项检测。"
echo "2. 普通 cron、普通 /tmp、普通 curl/wget、普通 authorized_keys 不再直接判定感染。"
...
echo "7. cron 或启动项只有出现 curl/wget 管道执行 sh/bash、下载恶意脚本、写入 root SSH 公钥等强特征时，才标记为疑似感染。"
echo "8. 小鸡公网远端 IP 数或公网连接数 >= ${CONN_THRESHOLD} 时，直接标记为疑似感染。"
echo "9. 使用 --nezha-only 可以只扫描安装/运行哪吒 agent 的小鸡。"
echo "10. 没有脚本能 100% 证明系统绝对干净。"
