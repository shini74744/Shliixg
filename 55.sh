...
# 基础路径定义
export SCRIPT_VERSION="15.3-alpine-stable-xlddg"
...
_pkg_install() {
    ...
    if command -v apk >/dev/null 2>&1; then
        ...
        _info "apk 安装: $pkgs"
        _with_timeout "$SB_INSTALL_TIMEOUT" apk add --no-cache --no-progress $pkgs || {
            _warn "apk add 失败，尝试刷新索引后重试一次。"
            _with_timeout "$SB_INSTALL_TIMEOUT" apk update && _with_timeout "$SB_INSTALL_TIMEOUT" apk add --no-cache --no-progress $pkgs
        }
...
_update_script() {
    ...
    if _with_timeout "$SB_DOWNLOAD_TIMEOUT" wget --timeout=20 --tries=2 -qO "$temp_script_path" "$SCRIPT_UPDATE_URL"; then
    ...
    for script_name in "${sub_scripts[@]}"; do
        ...
        for script_path in "${paths_to_check[@]}"; do
            if [ -f "$script_path" ]; then
                ...
                if _with_timeout "$SB_DOWNLOAD_TIMEOUT" wget --timeout=20 --tries=2 -qO "$temp_sub_path" "$script_url"; then
...
_do_update_xray() {
    ...
    if ! _with_timeout "$SB_DOWNLOAD_TIMEOUT" wget --timeout=20 --tries=2 -qO "$tmp_zip" "$download_url" || [ ! -s "$tmp_zip" ]; then
        _error "Xray 下载失败！"
        rm -rf "$tmp_dir"
        return 1
    fi
...
_advanced_features() {
    ...
    if [ ! -f "$script_path" ]; then
        ...
        if _with_timeout "$SB_DOWNLOAD_TIMEOUT" wget --timeout=20 --tries=2 -qO "$script_path" "$download_url"; then
            chmod +x "$script_path"
            _success "下载成功！"
        else
